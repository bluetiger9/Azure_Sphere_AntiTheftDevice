...
int GetIntercoreBuffers(BufferHeader **outbound, BufferHeader **inbound, uint32_t *bufSize)
{
	...
	if (inboundBufferSize != outboundBufferSize) {
		Uart_EnqueueString(UartCM4Debug, "GetIntercoreBuffers: Mismatched buffer sizes\r\n");
		return -1;
	}

	if (inboundBufferSize <= sizeof(BufferHeader)) {
		Uart_EnqueueString(UartCM4Debug, "GetIntercoreBuffers: buffer size smaller than header\n");
		return -1;
	}
	...
	uint32_t dataSize)
{
	...
	if (remoteReadPosition >= bufSize) {
		Uart_EnqueueString(UartCM4Debug, "EnqueueData: remoteReadPosition invalid\r\n");
		return -1;
	}
	...
	if (availSpace < sizeof(uint32_t) + dataSize + RINGBUFFER_ALIGNMENT) {
		Uart_EnqueueString(UartCM4Debug, "EnqueueData: not enough space to enqueue block\r\n");
		return -1;
	}
	...
	if (dataToEnd < sizeof(uint32_t)) {
		Uart_EnqueueString(UartCM4Debug, "EnqueueData: not enough space for block size\r\n");
		return -1;
	}
	...
	uint32_t *dataSize)
{
	...
	if (remoteWritePosition >= bufSize) {
		Uart_EnqueueString(UartCM4Debug, "DequeueData: remoteWritePosition invalid\r\n");
		return -1;
	}
	...
	if (availData < sizeof(uint32_t)) {
		if (availData > 0) {
			Uart_EnqueueString(UartCM4Debug, "DequeueData: availData < 4 bytes\r\n");
		}
	...
	if (dataToEnd < sizeof(uint32_t)) {
		Uart_EnqueueString(UartCM4Debug, "DequeueData: dataToEnd < 4 bytes\r\n");
		return -1;
	}
	...
	if (blockSize + sizeof(uint32_t) > availData) {
		Uart_EnqueueString(UartCM4Debug, "DequeueData: message size greater than available data\r\n");
		return -1;
	}

	// Abort if the caller-supplied buffer is not large enough to hold the message.
	if (blockSize > *dataSize) {
		Uart_EnqueueString(UartCM4Debug, "DequeueData: message too large for buffer\r\n");
		*dataSize = blockSize;
		return -1;
	}
